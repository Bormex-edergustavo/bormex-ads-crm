...
async function saveSale(sale, message) {
  try {
    ...
    if (isMetaQuotaError(error) && (await confirmRemoteSaleSaved(sale))) {
      render();
      toast("Venta registrada. Meta Ads sigue sin cuota, pero no bloqueó el guardado.");
      return true;
    }
...
function formatActionError(error) {
  ...
  if (isMetaQuotaError(error)) {
    return "Meta Ads agotó su cuota. Revisa el historial: si aparece ahí, la venta sí quedó guardada.";
  }
  return message;
}

async function confirmRemoteSaleSaved(sale) {
  if (!serverAvailable || !sale?.id) return false;
  try {
    const remoteState = await api("/api/state");
    mergeRemoteState(remoteState);
    saveState();
    return (state.sales || []).some((item) => item.id === sale.id);
  } catch {
    return false;
  }
}
