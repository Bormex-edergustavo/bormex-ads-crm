...
function formatActionError(error) {
  ...
  if (isMetaQuotaError(error)) {
    return "Meta Ads agotó su cuota. La venta no depende de Meta; recarga y revisa el historial antes de capturarla otra vez.";
  }
  return message;
}

async function recoverSaleSaveAfterMetaQuota(sale) {
  if (await confirmRemoteSaleSaved(sale)) return true;
  try {
    const remoteState = await api("/api/sales", { method: "POST", body: JSON.stringify(sale) });
    applyRemoteState(remoteState);
    return saleExistsInState(sale);
  } catch {
    return confirmRemoteSaleSaved(sale);
  }
}

async function confirmRemoteSaleSaved(sale) {
  if (!serverAvailable || !sale) return false;
  try {
    const remoteState = await api("/api/state");
    mergeRemoteState(remoteState);
    saveState();
...
function saleExistsInState(sale) {
  return (state.sales || []).some((item) => saleMatches(item, sale));
}
...
function saleMatches(item, sale) {
  ...
  const sameAd = normalizeAdId(item.adId) === normalizeAdId(sale.adId);
