...
const ADS_CONTROL_API_BASE = getAdsControlApiBase();
...
async function adsControlApi(path, options = {}) {
  const headers = {
    "content-type": "application/json",
    ...(options.headers || {}),
  };
  ...
  const response = await fetch(`${ADS_CONTROL_API_BASE}${path}`, {
    ...options,
    headers,
  });
  const contentType = response.headers.get("content-type") || "";
  const payload = contentType.includes("application/json")
    ? await response.json()
    : { error: await response.text() };
  if (response.status === 401) throw new Error("Ingresa con el código Ads para modificar Meta");
  if (!response.ok) throw new Error(payload.error || "No se pudo modificar Meta Ads");
  return payload;
}
...
function getAdsControlApiBase() {
  if (API_BASE.includes("/bormex-crm")) return API_BASE.replace(/\/bormex-crm\/?$/, "/bormex-ads-control");
  return "https://tnajelbyzkrifukfgnxv.functions.supabase.co/bormex-ads-control";
}
...
function scheduleFreshSync(delay = 1200) {
  window.setTimeout(() => {
    syncFromServer().then(render).catch(() => {});
  }, delay);
}
...
function renderConnectionStatus(config, remoteState, coexistenceSetup = null) {
  ...
  const whatsappTitle = document.getElementById("whatsappConnectionTitle");
  ...
  if (!serverAvailable) {
    ...
    if (whatsappTitle) whatsappTitle.textContent = "Reintentando conexión";
    if (whatsappText) whatsappText.textContent = "El backend no respondió en esta carga. El panel reintenta solo; si acabas de entrar, espera unos segundos o recarga.";
    if (coexistenceLink) setCoexistenceLink(coexistenceLink, "");
    if (coexistenceText) coexistenceText.textContent = "Coexistence se revisará cuando vuelva la conexión del panel.";
  ...
  if (config.whatsappWebhookConfigured) {
    ...
    if (whatsappTitle) whatsappTitle.textContent = "WhatsApp conectado";
    ...
    if (whatsappTitle) whatsappTitle.textContent = "Configuración pendiente";
...
function renderCoexistenceSetup(setup, link, text) {
  ...
  if (!setup) {
    ...
    if (text) {
      text.textContent = isSalesRole()
        ? "Coexistence solo se revisa con el código Ads; tu acceso de ventas sigue pudiendo atender CRM y registrar ventas."
        : "No pude revisar la configuración de Coexistence en este momento.";
    }
    return;
  }
...
async function syncAdsFromMeta() {
  ...
  try {
    ...
    state.rangeMetaStatus = remoteState.cacheStatus || "fresh";
    state.rangeLoadError = "";
    state.lastAdsRange = state.rangeMetaRange;
    state.lastAdsSync = remoteState.syncedAt || state.lastAdsSync || new Date().toISOString();
    saveState();
    render();
    toast(remoteState.warning || `Panel actualizado del ${range.startDate} al ${range.endDate}`);
    return true;
  } catch (error) {
...
function dashboardAds() {
  if (hasLoadedSelectedRange()) return state.rangeAds || [];
...
function getSaleAttribution(sale) {
  ...
  return findKnownAttributionForSale(sale);
}
...
function findConversationBySale(sale) {
  const sourceConversationId = String(sale?.sourceConversationId || "");
  if (sourceConversationId) {
    const direct = (state.conversations || []).find((conversation) => conversation.id === sourceConversationId);
    if (direct) return direct;
  }
  const phone = normalizePhone(sale?.phone);
  if (!phone) return null;
  return [...(state.conversations || [])]
    .filter((conversation) => normalizePhone(conversation.phone || conversation.contactId) === phone)
    .sort((a, b) => new Date(b.lastAt || 0) - new Date(a.lastAt || 0))[0] || null;
}

function findMessageAttributionForConversation(conversation, phoneValue = "") {
  const phone = normalizePhone(phoneValue || conversation?.phone || conversation?.contactId);
  const conversationId = String(conversation?.id || "");
  return [...(state.messages || [])]
    .filter((message) => {
      if (conversationId && message.conversationId === conversationId) return true;
      return phone && [message.from, message.to].some((value) => normalizePhone(value) === phone);
    })
    .map((message) => extractStoredAttribution(message, "message_referral"))
    .filter(Boolean)
    .sort((a, b) => Number(Boolean(b.adId)) - Number(Boolean(a.adId)))[0] || null;
}

function findKnownAttributionForSale(sale) {
  const conversation = findConversationBySale(sale);
  const lead = findLeadForSale(sale);
  return (
    extractStoredAttribution(conversation, "conversation") ||
    (lead ? { ...lead, source: "lead" } : null) ||
    findMessageAttributionForConversation(conversation, sale?.phone)
  );
}

function applyKnownSaleAttribution(sale) {
  const attribution = findKnownAttributionForSale(sale);
  if (!attribution) {
    sale.attributionSource = sale.attributionSource || "missing_meta_referral";
    return sale;
  }
  sale.adId = attribution.adId || "";
  sale.ad = attribution.ad || "";
  sale.adset = attribution.adset || "";
  sale.campaign = attribution.campaign || "";
  sale.campaignId = attribution.campaignId || "";
  sale.attributionSource = attribution.source || "auto_sale_attribution";
  return sale;
}
...
function getConversationAttribution(conversation) {
  const direct = extractStoredAttribution(conversation, "conversation");
  if (direct) return direct;
  const lead = getConversationLead(conversation);
  if (lead) return { ...lead, source: "lead" };
  return findMessageAttributionForConversation(conversation);
}
...
function getPerformance() {
  ...
  for (const ad of dashboardAds()) {
    ...
    map.set(key, {
      ...
      adId: ad.id || "",
      status: ad.status || "",
      effectiveStatus: ad.effectiveStatus || ad.effective_status || "",
      leads: 0,
      trackedLeads: 0,
  ...
  for (const lead of (state.leads || []).filter((item) => isDateInSelectedRange(item.date))) {
    ...
    if (!map.has(key)) {
      map.set(key, {
        ...
        adId: lead.adId || "",
        status: lead.status || "",
        effectiveStatus: lead.effectiveStatus || "",
        leads: 0,
        trackedLeads: 0,
        messages: 0,
        sales: 0,
  ...
  for (const spend of dashboardSpend().filter(spendMatchesSelectedRange)) {
    ...
    if (!map.has(key)) {
      map.set(key, {
        ...
        adId: spend.adId || "",
        status: spend.status || "",
        effectiveStatus: spend.effectiveStatus || "",
        leads: 0,
        trackedLeads: 0,
        messages: 0,
        sales: 0,
    ...
    row.status = row.status || spend.status || "";
    row.effectiveStatus = row.effectiveStatus || spend.effectiveStatus || "";
  ...
  for (const sale of getFilteredSales()) {
    ...
    if (!map.has(key)) {
      map.set(key, {
        ...
        effectiveStatus: attribution.effectiveStatus || "",
        leads: 0,
        trackedLeads: 0,
        messages: 0,
        sales: 0,
...
function getScalePlan(row) {
  ...
  return getBudgetPlan(row, "increase_budget");
}

function getBudgetPlan(row, action) {
  ...
  const step = Math.max(1, Math.round(currentBudget * SAFE_SCALE_RATE));
  const suggestedBudget = action === "decrease_budget" ? Math.max(1, currentBudget - step) : currentBudget + step;
  const difference = suggestedBudget - currentBudget;
  const percent = currentBudget ? Math.abs(difference / currentBudget) * 100 : 0;
  return {
...
function renderRecommendationAction(row) {
  ...
  const increasePlan = getBudgetPlan(row, "increase_budget");
  const decreasePlan = getBudgetPlan(row, "decrease_budget");
  const scalePlan = getScalePlan(row);
  if (!row.adsetId) {
    return `
      <div class="scale-action">
        ${badge}
        <small>Sin conjunto editable detectado</small>
      </div>
    `;
  }
  const isPaused = /paused/i.test(`${row.status} ${row.effectiveStatus}`);
  const statusAction = isPaused ? "activate" : "pause";
  const statusLabel = isPaused ? "Activar" : "Pausar";
  const statusClass = isPaused ? "secondary-button" : "danger-button";
  const copy = scalePlan ? scalePlanCopy(row, scalePlan) : "";
  return `
    <div class="scale-action">
      ${badge}
...
async function handleMetaAdsetAction(button) {
  const action = button.dataset.metaAdsetAction || "";
  const adsetId = button.dataset.adsetId || "";
  const adName = button.dataset.adName || "este conjunto";
  const targetBudget = Number(button.dataset.targetBudget || 0);
  const currentBudget = Number(button.dataset.currentBudget || 0);
  if (!adsetId) return;

  const prompt =
    action === "increase_budget"
      ? `¿Subir presupuesto de "${adName}" de ${money.format(currentBudget)} a ${money.format(targetBudget)}/día?`
      : action === "decrease_budget"
        ? `¿Bajar presupuesto de "${adName}" de ${money.format(currentBudget)} a ${money.format(targetBudget)}/día?`
        : action === "pause"
          ? `¿Pausar "${adName}"?`
          : `¿Activar "${adName}"?`;
  if (!confirm(`${prompt}\n\nEsto modifica Meta Ads en vivo.`)) return;

  const originalText = button.textContent;
  try {
    button.disabled = true;
    button.textContent = "Aplicando...";
    const result = await adsControlApi(`/api/adsets/${encodeURIComponent(adsetId)}`, {
      method: "POST",
      body: JSON.stringify({ action, targetBudget }),
    });
    applyMetaControlResult(result);
    saveState();
    render();
    toast(metaControlToast(result));
  } catch (error) {
    toast(error.message);
  } finally {
    button.disabled = false;
...
function applyMetaControlResult(result) {
  const adsetId = String(result?.adsetId || "");
  const after = result?.after || {};
  if (!adsetId) return;
  const patch = {
    adsetId,
    dailyBudget: Number(after.dailyBudget || 0),
    status: after.status || "",
    effectiveStatus: after.effectiveStatus || "",
  };
  const update = (row) => (row.adsetId === adsetId ? { ...row, ...patch } : row);
  state.rangeAds = (state.rangeAds || []).map(update);
  state.ads = (state.ads || []).map(update);
  state.rangeSpend = (state.rangeSpend || []).map(update);
  state.spend = (state.spend || []).map(update);
}

function metaControlToast(result) {
  const action = String(result?.action || "");
  const after = result?.after || {};
  if (action === "pause") return "Conjunto pausado en Meta";
  if (action === "activate") return "Conjunto activado en Meta";
  if (action === "increase_budget") return `Presupuesto subido a ${money.format(Number(after.dailyBudget || 0))}/día`;
  if (action === "decrease_budget") return `Presupuesto bajado a ${money.format(Number(after.dailyBudget || 0))}/día`;
  return "Cambio aplicado en Meta";
}
  ...
  tbody.innerHTML = [...state.sales]
    ...
    .map((sale) => {
      ...
      const attributionText = attribution
        ? `${escapeHtml(attribution.ad)} / ${escapeHtml(attribution.campaign)}`
        : sale.attributionSource === "missing_meta_referral"
          ? "Meta no envió origen"
          : "Sin coincidencia";
      return `
        <tr>
...
async function handleSaleSubmit(event) {
  ...
  if (saleExistsInState(sale)) {
    render();
    toast("Venta registrada. Ya estaba guardada en el historial.");
    form.reset();
...
function applyManualAdAttribution(sale, adIdInput, options = {}) {
  ...
  const matchedAd = findAdById(adId);
  sale.adId = adId;
  sale.campaign = matchedAd?.campaign || sale.campaign || "";
  sale.campaignId = matchedAd?.campaignId || sale.campaignId || "";
  sale.adset = matchedAd?.adset || sale.adset || "";
  sale.ad = adName(matchedAd) || sale.ad || "";
  sale.attributionSource = "manual_ad_id";
  return sale;
}
...
async function saveSale(sale, message) {
  try {
    ...
    if (isMetaQuotaError(error) && (await recoverSaleSaveAfterMetaQuota(sale))) {
      render();
      toast("Venta registrada. Meta Ads sigue sin cuota, pero no bloqueó la venta.");
      return true;
    }
    if (isMetaQuotaError(error)) toast(formatActionError(error));
    else toast(`No se guardó la venta: ${formatActionError(error)}`);
    return false;
  }
}
...
function formatActionError(error) {
  ...
  if (isMetaQuotaError(error)) {
    return "Meta Ads agotó su cuota. Revisé el servidor y no pude confirmar la venta; recarga y revisa el historial antes de capturarla otra vez.";
  }
  return message;
}

async function recoverSaleSaveAfterMetaQuota(sale) {
  if (await confirmRemoteSaleSaved(sale, { attempts: 5, delayMs: 350 })) return true;
  try {
    const remoteState = await api("/api/sales", { method: "POST", body: JSON.stringify(sale) });
    applyRemoteState(remoteState);
    return saleExistsInState(sale);
  } catch {
    return confirmRemoteSaleSaved(sale, { attempts: 4, delayMs: 650 });
  }
}

async function confirmRemoteSaleSaved(sale, options = {}) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      ...
      mergeRemoteState(remoteState);
      saveState();
      if (saleExistsInState(sale)) return true;
    } catch {
      // Meta quota errors can arrive after the sale write; retry the read before giving up.
    }
  }
  return false;
}

function saleExistsInState(sale) {
  return Boolean(findMatchingSale(sale));
}

function findMatchingSale(sale) {
  return (state.sales || []).find((item) => saleMatches(item, sale));
}

function saleMatches(item, sale) {
  if (sale.id && item.id === sale.id) return true;
  const samePhone = normalizePhone(item.phone) === normalizePhone(sale.phone);
  const sameDate = String(item.date || "") === String(sale.date || "");
  const sameAmount = Number(item.amount || 0) === Number(sale.amount || 0);
  const sameProducts = getSaleProducts(item).join("|") === getSaleProducts(sale).join("|");
  const itemAd = normalizeAdId(item.adId);
  const saleAd = normalizeAdId(sale.adId);
  const sameAd = !itemAd || !saleAd || itemAd === saleAd;
  return samePhone && sameDate && sameAmount && sameProducts && sameAd;
}

function isMetaQuotaError(error) {
  return /quota|cuota|limit|rate/i.test(String(error?.message || error || ""));
}
...
function mergeRemoteState(remoteState = {}) {
  ...
  state.lastAdsSync = remoteState.lastAdsSync || state.lastAdsSync;
  state.lastAdsRange = remoteState.lastAdsRange || state.lastAdsRange;
}
